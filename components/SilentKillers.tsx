...
const SilentKillers: React.FC = () => {
  return (
    <section id="killers" className="py-40 bg-evergreen text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="max-w-4xl mb-32 scroll-reveal">
          <span className="text-white/40 text-[10px] font-black uppercase tracking-[0.5em] mb-8 block">Threat Identification</span>
          <h2 className="font-space text-5xl md:text-8xl font-black leading-[0.9] tracking-tighter">
            Why Your Wealth <br />
            Is Under <span className="text-outline text-white/10 italic">Attack.</span>
          </h2>
          <p className="mt-10 text-white/60 text-xl font-light max-w-3xl leading-relaxed">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          {KILLERS.map((killer, idx) => (
            ...
            >
              ...
              <h4 className="text-[9px] font-black uppercase tracking-[0.4em] text-white/30 mb-5">{killer.label}</h4>
              <h3 className="font-space text-2xl md:text-3xl font-bold mb-8 leading-tight tracking-tight uppercase">{killer.title}</h3>
              <p className="text-white/50 leading-relaxed mb-12 flex-grow text-sm">
                {killer.copy}
              </p>
              
              <div className="pt-8 border-t border-white/5">
                 <div className="flex items-center gap-4">
                   <div className="p-2 bg-white/5 rounded-lg"><ShieldCheck className="text-white/20" size={16} /></div>
                   <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">{killer.stat}</span>
                 </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-40 text-center scroll-reveal">
          <p className="text-white/10 font-space text-[14vw] md:text-[11vw] font-black uppercase tracking-tighter leading-none select-none mb-4">
            RECLAIM CONTROL
          </p>
          <button className="relative z-20 mt-[-6vw] px-14 py-8 bg-white text-evergreen rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-all shadow-[0_30px_60px_-15px_rgba(255,255,255,0.2)]">
